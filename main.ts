...
class ConductorAgentWatchApp {
  ...
  private createTray() {
    ...
    this.tray.setToolTip('Conductor AgentWatch');
  ...
  private createScreenshotPath(): string {
    const screenshotsDir = path.join(os.homedir(), 'Pictures', 'ConductorAgentWatch-Screenshots');
    if (!fs.existsSync(screenshotsDir)) {
      fs.mkdirSync(screenshotsDir, { recursive: true });
    }

    const timestamp = new Date().toISOString().replace(/[:.]/g, '-').slice(0, -5);
    const filename = `AgentWatch-Screenshot-${timestamp}.png`;
...
// Initialize the app
const conductorAgentWatchApp = new ConductorAgentWatchApp();
conductorAgentWatchApp.initialize().catch((err) => Logger.error('Initialization failed:', err));
